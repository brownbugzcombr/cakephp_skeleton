(function($) {
    $.fn.featuredbox = function(custom) {
       ...
       /** Prepare featured box **/
        ...
        // Add extra divs
        box_innner.append('<p class="description" style="z-index:102"></p>');
        box.append('<ul class="navigation" style="z-index:101"></ul>');
        ...
        navigation_box.find("li").bind("click",
        function() {
       ...
       /** Slide transitions **/
        ...
        function slideBottomSlidesAnimation() {
            ...
            box.find(".box-slide2").animate({
                top: "0px"
            },
            settings.slidesSpeed, null, unlock_slide);
        }
        ...
        function lineSlidesAnimation() {
            ...
            settings.slidesSpeed);
            box.find(".box-slide1").animate({
                left: -settings.width + "px"
            },
            settings.slidesSpeed, null,
            function() {
       ...
       /** Description transitions **/
        function fadeDescriptionAnimation(new_content) {
            var description_box = box.find(".description");
            description_box.fadeOut(settings.descriptionSpeed,
            function() {
        ...
        function slideBottomDescriptionAnimation(new_content) {
            description_box.animate({
                top: settings.height + description_height + "px"
            },
            settings.descriptionSpeed, null,
            function() {
                if (new_content == "" || new_content == "undefined") {
                    description_box.hide();
                } else {
                ...
                description_box.html(new_content);
                description_box.animate({
                    top: description_top + "px"
                },
                settings.descriptionSpeed, null, unlock_description);
            });
        }

        function slideTopDescriptionAnimation(new_content) {
            description_box.animate({
                top: "-" + description_height + "px"
            },
            settings.descriptionSpeed, null,
            function() {
                if (new_content == "" || new_content == "undefined") {
        ...
        function slideRightDescriptionAnimation(new_content) {
            description_box.animate({
                left: settings.width + description_width + "px"
            },
            settings.descriptionSpeed, null,
            function() {
                if (new_content == "" || new_content == "undefined") {
       ...
       /** Extra functions **/
        ...
        function add_navigate_click_handlers() {
            ...
            if (settings.next != "") {
                box.find(settings.next).bind("click",
                function() {
                    if (!lock_description && !lock_slide) {
                        rotate();
                    }
            ...
            if (settings.previous != "") {
                box.find(settings.previous).bind("click",
                function() {
                    if (!lock_description && !lock_slide) {
                        rotate_back();
                    }
        ...
        function calc_description_box_size() {
            ...
            description_height = parseInt(description_box.css("padding-top").replace("px", "")) + parseInt(description_box.css("padding-bottom").replace("px", "")) + parseInt(description_box.css("height").replace("px", ""));

            description_width = parseInt(description_box.css("padding-left").replace("px", "")) + parseInt(description_box.css("padding-right").replace("px", "")) + parseInt(description_box.css("width").replace("px", ""));
        }
